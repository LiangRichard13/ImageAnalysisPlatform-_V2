...
def set_seed(seed, deterministic=False):
    ...
    Args:
        ...
        deterministic (bool): Whether to set the deterministic option for
            CUDNN backend, i.e., set `torch.backends.cudnn.deterministic`vscode-remote://ssh-remote%2B192.168.71.11/home/sstl/Disk1/data/PredFormer/openstl/utils/config_utils.py
...
def get_dataset(dataname, config):
    cfg_dataloader = dict(
        pre_seq_length=config.get('pre_seq_length', 10),
        aft_seq_length=config.get('aft_seq_length', 10),
        in_shape=config.get('in_shape', None),
        distributed=dist,
        use_augment=config.get('use_augment', False),
        use_prefetcher=config.get('use_prefetcher', False),
        drop_last=config.get('drop_last', False),
    )
    from openstl.datasets import dataloader_wrinkle, dataloader

    return dataloader_wrinkle.load_data(config['batch_size'], config['val_batch_size'], config['data_root'], config['num_workers'], **cfg_dataloader)
