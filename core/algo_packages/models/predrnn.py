...
class RNN(nn.Module):
    def __init__(self, num_layers, num_hidden, configs):
        ...
        for i in range(num_layers):
            in_channel = self.frame_channel if i == 0 else num_hidden[i - 1]#如果是第一个cell，iputchannel=图片通道数，如果不是，就等于上一层的通道数
            cell_list.append(
                SpatioTemporalLSTMCell(in_channel, num_hidden[i],height, width,configs.filter_size,
                                       configs.stride, configs.layer_norm)
            )
        self.cell_list = nn.ModuleList(cell_list)#将这个列表转化为模块列表。这样做的目的是让PyTorch知道cell_list中所有的SpatioTemporalLSTMCell都是网络的一部分，从而在计算梯度和更新权重时能够正确地处理这些SpatioTemporalLSTMCell。
