...
def cal_anomaly_maps(fs_list, ft_list, out_size=224):
    ...
    for i in range(len(ft_list)):
        ...
        ft = ft_list[i]
        a_map = 1 - F.cosine_similarity(fs, ft)
        # a_map = torch.sqrt(torch.sum((fs - ft) ** 2, dim=1))
        a_map = torch.unsqueeze(a_map, dim=1)
        a_map = F.interpolate(a_map, size=out_size, mode='bilinear', align_corners=True)
        a_map_list.append(a_map)
...
def evaluation_batch(model, dataloader, device, _class_=None, max_ratio=0, resize_mask=None):
    ...
    return [auroc_sp, auroc_px]#, ap_sp, f1_sp, ap_px, f1_px], #aupro_px]
...
def compute_pro_gpu(masks: torch.Tensor, amaps: torch.Tensor, num_th: int = 200) -> None:
    """Compute the area under the curve of per-region overlapping (PRO) with 0 to 0.3 FPR
    Args:
        masks (torch.Tensor): All binary masks in test. masks.shape -> (num_test_data, h, w)
        amaps (torch.Tensor): All anomaly maps in test. amaps.shape -> (num_test_data, h, w)
        num_th (int, optional): Number of thresholds
    """
    ...
    # Ensure the masks and amaps are float tensors for computations
    masks = masks.float()
    amaps = amaps.float()
    ...
    for th in torch.arange(min_th, max_th, delta).to('cuda'):
        # Perform thresholding using GPU
        binary_amaps = (amaps > th).float()

        pros = []
        for binary_amap, mask in zip(binary_amaps, masks):
            mask_cpu = mask.cpu().numpy()  # Temporarily move mask to CPU for regionprops
            for region in measure.regionprops(measure.label(mask_cpu)):
                axes0_ids = region.coords[:, 0]
                axes1_ids = region.coords[:, 1]
                # Get true positive pixels
                tp_pixels = binary_amap[axes0_ids, axes1_ids].sum().item()
                pros.append(tp_pixels / region.area)
        ...
        df = df.append({"pro": mean(torch.tensor(pros)).item(), "fpr": fpr, "threshold": th.item()}, ignore_index=True)
