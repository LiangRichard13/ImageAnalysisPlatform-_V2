...
class MVTecDataset(torch.utils.data.Dataset):
    def __init__(self, root, category=None, transform=None, gt_transform=None, phase='train'):
        self.root = root
        ...
        if category:
            # 如果指定了类别，使用新的数据结构
            if phase == 'train':
                self.img_path = os.path.join(root, category, 'good')
            else:
                self.img_path = os.path.join(root, category, 'good')
                self.fold_path = os.path.join(root, category, 'fold')
                self.gt_path = os.path.join(root, category, 'ground_truth')
        else:
            # 兼容旧的数据结构
            if phase == 'train':
                self.img_path = os.path.join(root, 'good')
            else:
                self.img_path = os.path.join(root, 'good')
                self.gt_path = os.path.join(root, 'fold')
        ...
        if self.category:
            ...
            if self.phase == 'train':
                ...
                img_paths = glob.glob(os.path.join(self.img_path, "*.png")) + \
                            glob.glob(os.path.join(self.img_path, "*.JPG")) + \
                            glob.glob(os.path.join(self.img_path, "*.jpg")) + \
                ...
                # 测试阶段加载good和fold图片
                # 加载good图片（正常图片）
                good_img_paths = glob.glob(os.path.join(self.img_path, "*.png")) + \
                                glob.glob(os.path.join(self.img_path, "*.JPG")) + \
                                glob.glob(os.path.join(self.img_path, "*.jpg")) + \
                                glob.glob(os.path.join(self.img_path, "*.bmp"))
                img_tot_paths.extend(good_img_paths)
                gt_tot_paths.extend([0] * len(good_img_paths))  # 正常图片没有ground truth
                tot_labels.extend([0] * len(good_img_paths))
                tot_types.extend(['good'] * len(good_img_paths))
                
                # 加载fold图片（异常图片）
                if hasattr(self, 'fold_path') and os.path.exists(self.fold_path):
                    fold_img_paths = glob.glob(os.path.join(self.fold_path, "*.png")) + \
                                    glob.glob(os.path.join(self.fold_path, "*.JPG")) + \
                                    glob.glob(os.path.join(self.fold_path, "*.jpg")) + \
                                    glob.glob(os.path.join(self.fold_path, "*.bmp"))
                    img_tot_paths.extend(fold_img_paths)
                    
                    # 为fold图片匹配对应的ground truth
                    fold_gt_paths = []
                    for fold_img_path in fold_img_paths:
                        fold_filename = os.path.basename(fold_img_path)
                        gt_filename = os.path.splitext(fold_filename)[0] + '.png'  # 假设ground truth是png格式
                        gt_path = os.path.join(self.gt_path, gt_filename)
                        if os.path.exists(gt_path):
                            fold_gt_paths.append(gt_path)
                        else:
                            fold_gt_paths.append(0)  # 如果没有对应的ground truth
                    
                    gt_tot_paths.extend(fold_gt_paths)
                    tot_labels.extend([1] * len(fold_img_paths))
                    tot_types.extend(['fold'] * len(fold_img_paths))
        else:
            # 兼容旧的数据结构
    ...
    def __getitem__(self, idx):
        ...
        if label == 0:
            ...
            # 异常图片，加载对应的ground truth mask
            if gt_path and gt_path != 0:
                gt = Image.open(gt_path).convert('L')  # 转换为灰度图
                gt = self.gt_transform(gt)
            else:
                # 如果没有ground truth，创建全零mask
                gt = torch.zeros([1, img.size()[-2], img.size()[-2]])
